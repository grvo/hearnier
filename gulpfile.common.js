...
exports.loaderConfig = function (emptyPaths) {
    ...
    (emptyPaths || []).forEach(function(m) {
        result.paths[m] = 'empty:';
    });

    return result;
};

var IS_OUR_COPYRIGHT_REGEXP = /copyright \(c\) grivo/;
...
exports.optimizeTask = function(
    ...
    return function() {
        ...
        bundle.bundle(entryPoints, loaderConfig, function (err, result) {
            if (err) {
                return bundles.emit('error', JSON.stringify(err));
            }

            // se um bundle se encerra incluindo em alguma das fontes dos copyrights
            // inserir source falsa no início de cada bundle com nosso copyright
            result.forEach(function (b) {
                var containOurCopyright = false;

                for (var i = 0, len = b.sources.length; i < len; i++) {
                    var fileContents = b.sources[i].contents;

                    if (IS_OUR_COPYRIGHT_REGEXP.test(fileContents)) {
                        containOurCopyright = true;

                        break;
                    }
                }

                if (containOurCopyright) {
                    b.sources.unshift({
                        path: null,
...
/**
 * envolver uglify e permitir função preserveComments para obter um "contexto" do arquivo para incluir nosso copyright
 */
