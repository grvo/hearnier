...
/**
 * checa se o objeto fornecido é compatível com a interface IAction
 * 
 * @param thing um objeto
 */
...
class ProxyAction extends Action implements IEventEmitter {
	...
	public addBulkListener(
        listener: IBulkListenerCallback
    ): ListenerUnbind {
...
export class ActionRunner extends EventEmitter implements IActionRunner {
	public run(action: IAction, context?: any): Promise {
		...
		this.emit(Events.EventType.BEFORE_RUN, {
            action: action
        });

		return Promise.as(action.run(context)).then((result: any) => {
			this.emit(Events.EventType.RUN, <IRunEvent> {
                action: action,
                result: result
            });
		}, (error: any) => {
