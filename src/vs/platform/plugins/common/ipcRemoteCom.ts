...
// importação de algumas propriedades common da base
        ...
        handle: (rawmsg) => {
            ...
            if (msg.seq) {
                if (!pendingRPCReplies.hasOwnProperty(msg.seq)) {
                    console.warn('foi respondido com seq desconhecido');
            ...
            if (!bigHandler) {
                throw new Error('mensagem recebida antes do big handler ser anexado!');
            }

            var req = msg.req;

            invokedHandlers[req] = invokedHandlers(rpcId, msg.method, msg.args);
